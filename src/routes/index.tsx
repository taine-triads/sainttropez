  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2">
      {showBubble && !dismissed && (
        <div className="relative max-w-[260px] animate-fade-in rounded-2xl rounded-br-sm bg-white p-4 pr-8 shadow-luxury">
          <button
            type="button"
            onClick={() => setDismissed(true)}
            aria-label="Fechar mensagem"
            className="absolute right-2 top-2 text-navy-deep/50 hover:text-navy-deep"
          >
            ✕
          </button>
          <p className="text-sm leading-snug text-navy-deep">
            Olá, em busca de um imóvel em Pinhais? <strong>Este apartamento pode ser seu!</strong>
          </p>
          <a
            href={WHATSAPP_URL}
            target="_blank"
      ...
      <a
        ...
        className="relative flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-luxury transition hover:scale-110 md:h-16 md:w-16"
      >
        {showBubble && !dismissed && (
